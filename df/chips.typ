...
所以它和加法器最根本的区别是：加法器输出的是“和”，比较器输出的是“大小关系”。考试中若给出芯片引脚图，只要你看到两组输入加上三种关系输出，通常就应想到比较器。
...
若只看功能，它可以理解为“把两个4位数送进去，直接告诉你谁大、谁小、是否相等”。因此，记忆芯片时最重要的不是死记所有管脚位置，而是先认出三类信号：
...
它的核心任务不是做加法，也不是比较大小，而是把“输入代码”翻译成“被选中的一路输出”。所以看到译码器时，可以先把它理解成一个 *代码到线路* 的转换器。
...
因此，和前面的加法器、比较器一样，记芯片时不必先死记每个管脚编号，而应先认出：*4位选择输入 + 16路输出 + 使能端*。
...
例如，若输入代码对应十进制 $5$，则第5路输出被选中，其余15路不被选中。这就是 “1-of-16” 名称的直接含义。
...
所以BCD-to-decimal decoder可以理解成：*4位输入不再表示任意16种状态，而是只表示十进制数字0到9。*

#method[工作方式][
  当输入为某个合法BCD码时，十条输出线中与该十进制数字对应的一路被选中。
]

例如，输入为表示十进制 $7$ 的BCD码时，数字 $7$ 对应的输出线有效，其余输出线无效。若输入不是合法BCD码，则通常不作为正常十进制数字来解释。

#note[芯片怎么认][
  记忆这类芯片时，先抓住“4位BCD输入 + 10路十进制输出”这个功能特征。
]

只要你看到输入端是4位，而输出端只有10路，不是16路，就应立刻想到它不是普通的4线到16线译码器，而是BCD到十进制译码器。
...
#method[为什么只要7条输出][
  七段数码管显示一个数字，本质上是决定7个发光段哪些点亮、哪些熄灭，因此不需要10条输出线分别代表10个数字。
]

例如，显示某个十进制数字时，译码器会根据输入的BCD码，直接给出 $a$ 到 $g$ 七段的开关状态。也就是说，它输出的不是“这个数字是谁”，而是“哪些段该亮”。

#note[和前一小节的区别][
  BCD-to-decimal decoder是“选某一路数字输出”，BCD-to-7-segment decoder是“直接生成显示该数字所需的段码”。
]

所以这类芯片特别适合直接接七段显示器，省去了再把十进制输出重新组合成段信号的麻烦。
...
所以编码器最常见的识别特征就是：*输入多，输出少；输出不是选中的线路，而是该线路的编号。*
...
所以考试若给出芯片管脚图，只要你看到输入端很多，而输出端只有4位，并且功能和数字编号有关，就要想到decimal-to-BCD encoder。
...
从图左也能直接看出这一点：最低3位的Gray输出都来自异或门，而最高位G3直接由最高位binary输入送出。
...
真正发生变化的是后面的各位，而这些位都可以用异或关系写出，所以这类码制转换器常用异或门实现。
...
图中可以看到两组输入分别是1I0到1I3与2I0到2I3，公共选择线是S0、S1，输出分别是1Y与2Y。这就是dual four-input的直接含义：*一片芯片里装了两个4选1。*
...
所以看到“8路输入 + 3条选择线 + 低有效使能 + 正反两个输出”，基本就能认出这一类8选1multiplexer。
...
这和前面加法器、比较器的级联思路不同。这里传递的不是进位，也不是大小关系，而是把“更高一位的选择”变成“启用哪一个子multiplexer”。
...
这就是动态显示或扫描显示的基本原理：*段信号复用，位选轮流打开。*

#remark[图中真正复用的是什么][
  被复用的是BCD到7段译码这一整套通路，而不是给每一位都单独放一套完整译码器。
]

这样可以减少器件数量，但代价是必须有时序切换与位选控制。
...
这部分是multiplexer在考试里非常常见的用法。它不只是“选数据”，还可以*用选择动作来实现组合逻辑函数*。
...
例如，用8选1multiplexer实现三变量函数时，通常把三个变量直接作为选择线，这样每个数据输入只需接成0或1；若用4选1multiplexer实现三变量函数，则常把其中两个变量作选择线，剩下那个变量决定各数据端接0、1、 $X$ 或 $bar(X)$。

#note[和真值表的对应关系][
  multiplexer的每个数据输入端，本质上就对应选择线某一种取值下，函数应该输出什么。
]

所以做题时可以把multiplexer看成一个“按地址读函数值”的器件：选择线给出地址，数据输入端预先存好该地址下应该输出的内容。
...
它的含义是：选择线决定“数据输入究竟送到哪一路输出”，未被选中的其余输出保持无效。
...
所以demultiplexer也可以记成一句话：*选择线给出编号，输入数据就被送到对应编号的输出线。*

#note[和multiplexer的关系][
  multiplexer是多入一出，demultiplexer是一入多出。
]

两者都由选择线控制，但multiplexer在很多输入中挑一路送到输出，而demultiplexer则把唯一输入导向很多输出中的一路。
...
所以看到“4条选择线 + 16路输出 + 一路Data in”，就应想到这是1线到16线demultiplexer，哪怕它内部是借助decoder结构实现的。
...
它的核心思想很简单：不去判断数据内容本身对不对，而是只统计整组数据中 1 的个数是奇数还是偶数。这样虽然不能定位哪一位错了，但能快速发现“这一组数据有问题”。
...
图中输入端是A到I，共9路输入；输出端是ΣEven和ΣOdd。最重要的识别抓手就是：*很多输入位 + 两个奇偶结果输出*。
...
所以这片芯片本质上是在回答一个问题：*这组输入里1的个数到底是偶数还是奇数？*
...
#method[低有效S-R latch的4种状态][
  + SET： $bar(S) = 0$、 $bar(R) = 1$，输出变成 $Q = 1$
  + RESET： $bar(S) = 1$、 $bar(R) = 0$，输出变成 $Q = 0$
  + 保持：两输入都为1，输出保持原状态
  + 非法：两输入都同时为0，回到无效状态后最终结果不确定
]

图中的波形强调的正是“瞬时有效输入只负责把状态改掉，而状态会继续被记住”。所以S-R latch最本质的能力不是算逻辑，而是*存住上一次被置成什么状态*。
...
图中已经明确指出：第1组有1S1、1S2两个置位端，第3组有3S1、3S2两个置位端。这是74HC279A最容易考细节的地方。
...
这样背的好处是：考试若只给出11脚或12脚，你会立刻想到它们都是第3组的置位端；若只给出14脚，则应立刻联想到第4组复位端。
...
所以gated S-R latch的关键记忆点不是新的真值表，而是：*先看EN，再看S和R。*
...
也就是说，这种器件在EN打开时是“透明”的，输入怎么变，输出就怎么跟；而EN关掉后，它会把最后一刻的数据锁住。
...
图中的C1、C2、C3、C4本质上就是4个latch各自的控制端，其中C1与C2共用13脚，C3与C4共用4脚。
...
这正是74HC75和“4个完全独立latch”最容易混淆的地方。题目若只给出13脚、2脚、15脚，你要能立刻判断那是在考第1、2组共享控制下的输入输出关系。
...
因此，D flip-flop可以理解成一个“在时钟边沿做快照”的1位存储器。它不是一直跟着D变化，而是只在采样那一瞬间更新。
...
也正因为如此，图中的两个例子才会分别在上升沿到来时完成置位或复位，而不会在整个高电平期间持续变化。
...
#remark[异步预置与清零要和普通时钟输入分开记][
  图中加入的 $bar(P R E)$ 与 $bar(C L R)$ 说明：某些D flip-flop除了常规的D与CLK以外，还带有异步置位和异步清零端。这两类输入一旦有效，会直接改写输出，而不必等待时钟边沿。
]
...
图中方块内写着S与R，就是在提醒你：PRE负责异步置位，CLR负责异步清零，而且它们的优先级高于普通时钟采样。
...
所以J-K flip-flop和S-R latch最本质的区别就在最后一行：S-R的“双有效”是非法，J-K的“双有效”是翻转。
...
如果题目问“13脚来的脉冲为什么在高到低时才起作用”，答案就不该从J、K去想，而应直接从负边沿触发去想。
...
这就是最基本的register思想。先用清零端把所有位拉到0，再在下一次时钟边沿把外部4位数据整体装入。
...
#overview[flip-flop天然适合做分频器][
  若把D flip-flop接成 $D = bar(Q)$，或把J-K flip-flop接成 $J = K = 1$，那么每来一个有效时钟边沿，输出就翻转一次。
]
...
图中的 $Q_A$ 变化更快，因此是低一级分频输出； $Q_B$ 变化更慢，因此代表更高一级的分频结果。
...
所以，图区分这两类器件最直接的方式不是看内部门电路，而是看“第二个触发脉冲落在输出高电平期间时，输出到底会不会被延长”。
...
只要认出1脚与6脚是一对互补输出，9、10、11三脚负责定时，整片器件的骨架就已经抓住了。
...
比起74121，74LS122多出来的一个明显抓手就是5脚低有效CLR。做题时这往往是区分两类器件最快的办法。
...
这说明retriggerable one-shot不仅能单独发脉冲，还很适合搭顺序定时链。
...
因此，555作astable时真正来回变化的核心量不是输出脚本身，而是电容电压在上下两个阈值之间周期摆动。

#image("/assets/image-72.png")

#note[频率由什么决定][
  Figure 7-58直接告诉你：振荡频率主要由 $C_1$ 与 $R_1 + 2 R_2$ 共同决定。
]
