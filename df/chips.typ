...
于是3 位异步二进制计数器的状态顺序就是000、001、010、011、100、101、110、111，然后回到000。读波形时，*谁变得最快，谁就是低位*，这一点比死背状态表更稳。
...
#overview[74HC93是最典型的4 位异步二进制计数器][
  它内部并不是一整块4位统一结构，而是拆成一个divide-by-2段和一个divide-by-8段。
]

这正是它最爱考的地方。只要你看到两根时钟输入CLK A、CLK B，再看到两根复位输入RO(1)、RO(2)，就应想到：这片器件允许把最低位单独拿出来用，也允许把它和高3位串起来组成4 位ripple counter。

#definition[器件结构与功能][
  + CLK A驱动最低位divide-by-2段
  + CLK B驱动高3位divide-by-8段
  + RO(1) 与RO(2) 同时为有效时，异步清零到0000
  + 若把 $Q_0$ 外接到CLK B，就得到完整的4 位mod-16计数器
]
...
#summary[这一小节要记住的点][
  ...
  + 完整4 位计数时，要把 $Q_0$ 接到CLK B
  + RO(1)、RO(2) 同时有效时异步清零
  + 74HC93常考mod-16与 decade两种接法
]
...
#overview[异步十进制计数器的本质是“先数到10，再立刻回0”][
  它不是天然只会数0 到9，而是先按二进制一路数上去，再把十进制10译出来做异步回收。
]
...
图中是mod-12例子。因为12的二进制是1100，所以只要把对应状态译出来，一旦计到12就清零，实际循环就停在0 到11。
...
这就是同步binary counter最值得记的规律。更高位是否翻转，并不是等低位去“敲它一下”，而是看低位是否同时组成了全1条件。于是4 位、5位、6位的规律也就顺下来了。

#image("/assets/image-85.png")

#note[表格里为什么老写No Change和 Toggle][
...
#strategy[考试速记][
  binary synchronous counter = 共用同一时钟；第 $n$ 位在所有低位全为1 时翻转。
]
...
#overview[同步decade counter的关键不是“检测10后拉清零”，而是“在9 的下一拍就安排回到0”][
...
#overview[74HC163是最常考的4 位同步二进制计数器][
...
背管脚时，先把1、2、9、10这四个控制脚抓出来，再把3 到6 的数据输入和11到 14的输出对起来记，最后单独补上15脚的RCO。这样比从1 背到16更不容易乱。
...
#definition[翻转条件怎么记][
  + 向上计数：更高位在所有低位都为1 时翻转
  + 向下计数：更高位在所有低位都为0 时翻转
  + 改变的是组合逻辑，不是时钟结构
]
...
#overview[Gray code counter的目标不是普通二进制递增，而是保证相邻状态只变1 位][
  这样在状态切换时更不容易因为多位同时翻转而引起误判。
]

它和前面code converter一节正好互相照应。那里讲的是binary和 Gray之间怎么换；这里讲的是如何直接按Gray序列去计数。

#method[读这种图时先抓结果，不先硬推门表达式][
  + 每个时钟边沿只允许1 位翻转
  + 典型3 位序列可读成000、001、011、010、110、111、101、100
  + 走完8 个状态后回到000
]

所以，Gray counter最关键的识别点不是“门多不多”，而是“状态切换时有没有同时改好几位”。只要题目强调相邻状态只变1 位，就应优先联想到Gray code counter。

#strategy[考试速记][
  synchronous Gray code counter = 共用时钟，但输出按Gray序列走，相邻状态只改1 位。
]
...
这类题最容易看懵，但其实思路很统一：*不是从0 开始数，而是先装一个起始值，再数到自然溢出。* 只要把“总状态数减去预置值”这一步想明白，分频比就出来了。

#summary[这一节要记住的点][
  + cascaded counters的总模数等于各级模数相乘
  + decade级联常用于divide-by-100、divide-by-1000
  + 74HC190级联时要分清MAX/MIN与 RCO
  + 并行预置计数法适合做非常规大分频比
]
...
#overview[计数器加译码器，最直观的效果就是“按编号轮流选通输出”][
  BCD counter给出4 位状态，BCD-to-decimal decoder把它翻成0 到9 中的一路有效。
]
...
#strategy[考试速记][
  counter + decoder若有毛刺，就给decoder加 strobe；本质是让译码只在稳定窗口内有效。
]
...
图中的Decode 9和 Decode 12正是在干这件事。它们不是普通显示译码，而是专门监控“什么时候该让十位翻转”“什么时候该把整组小时数装回01”。因此，12小时制时钟的小时部分总是比秒和分复杂得多。
...
#overview[计数器配合multiplexer，可以把并行数据一位一位串行送出][
  3位计数器不断扫过0 到7，MUX就按这个地址依次选出 $D_0$ 到 $D_7$。
]
...
#remark[时序图要会一句话概括][
  第1组8 个时钟周期依次送出第1个字节的 $D_0$ 到 $D_7$；下一组8 个周期再送出下一个字节。本质上是“counter在扫地址，MUX在按地址串行吐位”。
]
